...
import {
  Globe, Briefcase, Award, Linkedin, Instagram, Target,
  Lightbulb, ArrowRight, Sparkles, TrendingUp, Users,
  Building2, Menu, X
} from "lucide-react";
    ...
    <div className="bg-white min-h-screen font-sans text-slate-900 selection:bg-blue-100 selection:text-blue-900">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-medium text-gray-600">
            ...
            <Link
              ...
              className={`transition ${isActive('/about')
                ? 'text-gray-900 font-semibold'      // ← your requested black + bold
                : 'hover:text-blue-600'}`}
            >
              About
            </Link>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-16 left-0 w-full bg-white border-b border-gray-100 shadow-xl px-6 py-8 flex flex-col gap-6 z-40 animate-in slide-in-from-top-2">
            ...
            <Link
              href="/articles"
              onClick={() => setIsMobileMenuOpen(false)}
              className={isActive('/articles') ? 'font-semibold text-blue-600' : ''}
            >
              Articles
            </Link>
            <Link
              href="/about"
              onClick={() => setIsMobileMenuOpen(false)}
              className={isActive('/about') ? 'font-semibold text-gray-900' : ''}   // ← black when active
            >
              About
            </Link>
          </div>
      ...
      <footer className="max-w-7xl mx-auto px-6 py-12 flex flex-col md:flex-row justify-between items-center text-slate-400 text-sm border-t border-slate-100 mt-12">
        ...
        <div className="flex gap-6 mt-4 md:mt-0">
          ...
          <a href="mailto:ukonisophia@gmail.com" className="p-2 bg-slate-100 rounded-full text-slate-600 hover:bg-slate-900 hover:text-white transition-all"><SiGmail size={20} /></a>
        </div>
      </footer>
